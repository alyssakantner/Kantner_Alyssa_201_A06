...
<https://github.com/alyssakantner/Kantner_Alyssa_201_A06>
:::
...
```{r}
#extract value via indexing
player_info[[2]][4]
#extract value via names
...
```{r}
#create vector with player heights
vec_height <- c(78, 81, 78, 86, 85)

#loop through and add each height to the player list
for(i in 1:length(player_info)){
  player_info[[i]]$height <- vec_height[i]
}
...
for(player in names(player_info)){
  #calculate each player's points per game
...
```{r}
#use sapply to round points per game to two decimal places
ppg_rounded <- sapply(points_per_game, function(x) round(x, 2))
...
#loop through each column starting at 3
for(element in 3:ncol(max_temps)){
  #calculate the average temperature in each column
  avg_temp <- (sum(max_temps[ ,element]))/ nrow(max_temps)
  #add this average to a list
  temp_vec <- append(temp_vec, avg_temp)
}
...
```{r}
#create vector that takes medians of each column
median_vec <- apply(colorado[-1:-2], 1, median)

#find the year that the max median occurs at
max_year <- which.max(median_vec)
#find the value of the max median
max_median <- max(median_vec)

#find the year that the min median occurs at
min_year <- which.min(median_vec)
#find the value of the max median
min_median <- min(median_vec)

#print final statement
print(paste0("The maximum median annual temperature of ", max_median, " occurred in ", colorado[max_year, 2], " and the minimum median annual temperature of ", min_median, " occurred in ", colorado[min_year,2], "."))
...
sum_of_digits <- function(n){
  digit_count <- n %% 10
  #check if we have run through every digit
  if(n ==0 ){
    return(0)
  }else{
    #add each of the digits together
    return(digit_count + sum_of_digits(n %/% 10))
  }
}
